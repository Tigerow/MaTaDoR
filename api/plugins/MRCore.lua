...
local lang = redis:get(hash) 
	...
	keyboard.inline_keyboard = {
		...
		{
			{text = '> Sudo Uѕ 👑', callback_data = '/matador:'..GP_id}
		},
		{
  ...
  elseif lang then
	...
	keyboard.inline_keyboard = {
		...
		{
			{text = '>  سازنده ربات 👑', callback_data = '/matador:'..GP_id}
		},
		{
...
if not is_mod1(matches[2], msg.from.id) then
   ...
   elseif lang then
	...
	local text = [[
`به بخش راهنمای ربات مگا اسپید خوش آمدید`🤖🌹
...
🆔 : [SudoMegaSpeed](Telegram.Me/sudomegaspeed)
🆔 : [MessengerSudoBot](Telegram.Me/messengersudobot)
	]]
	keyboard = {} 
	keyboard.inline_keyboard = {
		{
...
if not lang then
    text = [[[♨️ MegaSpeed](Telegram.me/megaspeeed)
...
_=-=-=-=-=-=-=-=-=-=-=-=_
	...
	elseif lang then
	text = [[[♨️ مگا اسپید](Telegram.me/megaspeeed)
...
`تنظیم تعداد کارکتر`
_=-=-=-=-=-=-=-=-=-=-=-=_
▪️*خفه* _[username|id|reply]_
`سکوت کردن کاربر`
_=-=-=-=-=-=-=-=-=-=-=-=_
▫️*ان خفه* _[username|id|reply]_
`حذف سکوت کاربر`
_=-=-=-=-=-=-=-=-=-=-=-=_
...
▫️*Lock*
[`link ~ join ~ tag ~ fosh ~ username ~ edit ~ arabic ~ webpage ~ bots ~ spam ~ flood ~ markdown ~ mention ~ pin ~ cmds  ~ username ~ english ~ gif ~ photo ~ document ~ sticker ~ keyboard ~ video ~ text ~ forward ~ location ~ audio ~ voice ~ contact ~ all`]
...
▫️*قفل*
[لینک ~ ویرایش ~ تگ ~ فحش ~ نام کاربری ~ عربی ~ وب ~ ربات ~ هرزنامه ~ پیام مکرر ~ فراخوانی ~ سنجاق ~ دستورات ~ ورود ~ فونت ~ انگلیسی ~ گیف ~ عکس ~ فایل ~ استیکر ~ صفحه کلید ~ فیلم ~ متن ~ فوروارد ~ مکان ~ اهنگ ~ ویس ~ مخاطب ~ کیبورد شیشه ای ~ بازی ~ خدمات تلگرام ~ گروه]
`قفل موارد بالا به صورت معمولی`
▪️*بازکردن*
[لینک ~ ویرایش ~ تگ ~ فحش ~ نام کاربری ~ عربی ~ وب ~ ربات ~ هرزنامه ~ پیام مکرر ~ فراخوانی ~ سنجاق ~ دستورات ~ ورود ~ فونت ~ انگلیسی ~ گیف ~ عکس ~ فایل ~ استیکر ~ صفحه کلید ~ فیلم ~ متن ~ فوروارد ~ مکان ~ اهنگ ~ ویس ~ مخاطب ~ کیبورد شیشه ای ~ بازی ~ خدمات تلگرام ~ گروه]
`باز کردن قفل موارد بالا`
...
if matches[1] == '/matador' then
	...
	keyboard.inline_keyboard = {
		{
			{text = "> Sudo", callback_data="/mahdiroo:"..matches[2]}
		},
		{
			{text = '> GitHub ', url = 'https://www.github.com/MRMahDiRoO'}
		},
		{
			{text= '> Ɓαcк 🔙' ,callback_data = '/option:'..matches[2]}
		}				
   ...
   elseif lang then
	...
	keyboard.inline_keyboard = {
		{
			{text = "> سودو", callback_data="/mahdiroo:"..matches[2]}
		},
		{
			{text = '> گیت هاب  ', url = 'https://www.github.com/MegaSpeed'}
		},
		{
			{text= '> بازگشت 🔙' ,callback_data = '/option:'..matches[2]}
		}				
...
local text = [[
*》SudoMegaSpeed Information《*
_》Age :_ *20*
_》Name :_ * Sudo*
_》City :_ *Te*
*-------------------------*
*》GitHub :《*
》[MegaSpeed](Github.Com/megaspeed)
*-------------------------*
*》Bridges :《*
_》Pv : _[SudoMegaSpeed](Telegram.Me/sudomegaspeed)
_》PvResan : _[MessengerSudoBot](Telegram.Me/messengersudobot)
*-------------------------*
*》Expertise :《*
